# Ilkka Hirvelä — Portfolio

Personal portfolio site showcasing my game development projects and technical skills.

[![Live Site](https://img.shields.io/badge/PLAY-VISIT%20PORTFOLIO%20%E2%96%B8-FFB454?style=for-the-badge&labelColor=0B0908)](https://ilkkahirvela.github.io/Portfolio/)

---

## Projects

| Project | Year | Description |
|---------|------|-------------|
| [Coinflippa](https://ilkkahirvela.github.io/Portfolio/project.html?id=coinflippa) | 2026 | A 1000-player elimination game built on the simplest game there is: the coin flip. A polished mobile game in Unity with a Node.js real-time multiplayer stack. |
| [Kindling](https://ilkkahirvela.github.io/Portfolio/project.html?id=kindling) | 2026 | Systems-driven top-down adventure game built around light-based mechanics. Developed in Unity as a team project. |
| [Maze Solver](https://ilkkahirvela.github.io/Portfolio/project.html?id=mazesolver) | 2025 | Generates and solves mazes with real-time animated visualization. Built in C++ with SFML and Dear ImGui. |
| [y2k: Entity](https://ilkkahirvela.github.io/Portfolio/project.html?id=y2kentity) | 2025 | Turn-based tactics RPG built in Unity as a student team project. I built the grid system, the camera system, and the combat UI. |
| [ChessMachine3000](https://ilkkahirvela.github.io/Portfolio/project.html?id=chessmachine3000) | 2025 | Terminal-based chess engine with a minimax AI opponent. Supports human vs bot and bot vs bot modes with full chess rule coverage. |
| [Viridianland](https://ilkkahirvela.github.io/Portfolio/project.html?id=viridianland) | 2024 | Fast-paced top-down shooter built as a school project in Unity. |

> More coming soon.

---

## Tech Stack

**Engines:** Unity, Godot, Roblox Studio  
**Languages:** C#, C++, Python, JavaScript, Lua, GDScript, SQL, HTML, CSS  
**Tools:** AI, Aseprite, Git, NodeJS, Blender, Photoshop

---

## Contact

- Email: [hirvela.ilkka@gmail.com](mailto:hirvela.ilkka@gmail.com)
- GitHub: [github.com/ilkkahirvela](https://github.com/ilkkahirvela)
- Itch.io: [ilkkahi.itch.io](https://ilkkahi.itch.io)
