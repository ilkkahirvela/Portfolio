    ...
    content: {
      ...
      sections: [
        ...
        },
        {
          ...
          p: [
            "The visuals lean heavily on custom shaders, so the performance work was done on real Android hardware rather than in the editor. An on-device frame-time overlay and a set of live tuning knobs were built first, so every change could be A/B tested where it actually runs, and the frame budget was traded line by line against measurement instead of intuition. Readings were taken heat-soaked rather than cold, because thermal throttling changes which cost dominates.",
            "The fullscreen background shader is the heaviest thing the game draws, so it took the deepest cuts. It was decoupled from the frame loop entirely: it renders into its own low-resolution target at a fraction of the display's pixels, refreshes every second frame, and is composited back to full resolution by a camera-less blit. The motion is slow and painterly by design, so it absorbs both reductions with little visible loss. The rest of the frame is cheap by construction: one 3D coin plus UI.",
            "The remaining wins came off the coin and the post stack. The coin's engraving lookups were baked offline into a channel map, so the shader performs one texture read instead of up to eighteen. Once the background stopped being the bottleneck, post-processing became it, so bloom dropped to quarter resolution and three blur iterations, and render scale now steps down per screen where the coin is small or peripheral. The upscaler was swapped from FSR to bilinear, which measured cheaper at the resolutions the game actually renders.",
            "The frame-rate target behaved backwards. Capping at 60 let the device governor downclock until routine spikes missed vsync, so the shipping default is 120fps on high-refresh panels, where the clocks stay pinned. The baseline target is still 60 on mid-range hardware; on a Galaxy S22 Ultra the build holds a stable 120."
          ]
        },
        {
          h2: "Reflection",
          p: [
            "The strongest outcome was the mobile performance work: profiling on real devices and trading the frame budget deliberately to keep shader-heavy visuals smooth. That transfers to every mobile project after this one. The second is the visual bar itself, the coin, its shader and the UI around them landing closer to a commercial product than a student prototype.",
            "Keeping the game simple is what made the rest possible: the mechanics took little design effort, so the hours went into the server, the performance work, and the polish instead. It also put a ceiling on how interesting the game could be. The only decisions it offers are cosmetic. What came out is a polished technical demo more than an entertainment product.",
            "Measured against what I set out to learn, it delivered: mobile optimization, a real-time multiplayer architecture built from the socket up, a full progression economy, and a first-hand sense of where solo development time really goes.",
            "The multiplayer stack is built. What stands between the game and a store release is deployment: hosting, a database, and the store pipeline."
          ]
        }
      ],
      gallery: [
